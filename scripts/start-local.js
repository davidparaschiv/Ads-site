...
const temporary=await mkdtemp(path.join(tmpdir(),'da-start-'));
const readyFile=path.join(temporary,'ready.json');
const token=randomBytes(20).toString('hex');
let exited=false,closing=false;
const child=spawn(windows?'cmd.exe':'npm',windows?['/d','/s','/c','npm run dev']:['run','dev'],{stdio:'inherit',detached:!windows,env:{...process.env,DA_READY_FILE:readyFile,DA_LAUNCH_TOKEN:token}});
...
if(ready&&!closing){
  ...
  if(process.env.DA_NO_BROWSER!=='1'){
