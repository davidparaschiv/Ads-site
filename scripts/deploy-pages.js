...
try{
  ...
  if(!['da','d','yes','y'].includes(confirmation)){console.log('Publicare anulată.');process.exitCode=0;}
    ...
    const files=['index.html','styles.css','script.js','images','vendor'];
    ...
    temporary=await mkdtemp(path.join(tmpdir(),'da-pages-'));
    ...
    const marker='da-'+Date.now()+'-'+randomBytes(4).toString('hex');
    const index=await readFile(path.join(temporary,'index.html'),'utf8');
    await writeFile(path.join(temporary,'index.html'),index.replace('</head>','<meta name="da-publicare" content="'+marker+'">\n</head>'));
    git(['config','user.name',name],temporary);git(['config','user.email',email],temporary);
    git(['add','--all'],temporary);git(['commit','-m','Publicare D&A Agency'],temporary,true);
